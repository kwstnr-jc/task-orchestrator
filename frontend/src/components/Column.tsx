    ...
    <Droppable droppableId={state}>
      {(provided, snapshot) => (
        <div
          ...
          className={`flex-shrink-0 w-full lg:w-80 bg-gray-900 rounded-lg border transition-colors ${
            snapshot.isDraggingOver
              ? "border-blue-500 bg-gray-800"
              : "border-gray-800"
          }`}
        >
          <div className="sticky top-0 bg-gray-900 border-b border-gray-800 px-4 py-3 z-10 hidden lg:block">
            <h2 className="font-semibold text-gray-100">{STATE_LABELS[state]}</h2>
            <p className="text-xs text-gray-500">{tasks.length} tasks</p>
          </div>
          <div className="p-3 space-y-2 lg:max-h-[calc(100vh-10rem)] overflow-y-auto">
